...
1. **Calculadora de Soma:** Crie um endpoint `POST /api/calculadora/soma`.
    * **Parâmetros:** `{"numero1": 5, "numero2": 10}`
    * **Resposta:** `{"resultado": 15}`

2. **Área do Retângulo:** Crie um endpoint `POST /api/geometria/retangulo`.
    * **Parâmetros:** `{"base": 10, "altura": 5}`
    * **Resposta:** `{"area": 50}`

3. **Conversor de Temperatura:** Crie um endpoint `POST /api/conversores/temperatura`.
    * **Parâmetros:** `{"celsius": 30}`
    * **Resposta:** `{"fahrenheit": 86.0}`

4. **Cálculo de Média:** Crie um endpoint `POST /api/escola/media-simples`.
    * **Parâmetros:** `{"nota1": 7, "nota2": 8, "nota3": 6}`
    * **Resposta:** `{"media_aritmetica": 7.0}`

5. **Juros Simples:** Crie um endpoint `POST /api/financas/juros-simples`.
    * **Parâmetros:** `{"capital": 1000, "taxa_mensal": 0.05, "meses": 12}`
    * **Resposta:** `{"valor_juros": 600}`

6. **Calculadora de IMC:** Crie um endpoint `POST /api/saude/imc`.
    * **Parâmetros:** `{"peso_kg": 70, "altura_m": 1.75}`
    * **Resposta:** `{"imc": 22.86}`

7. **Conversor de Minutos:** Crie um endpoint `POST /api/conversores/tempo-horas`.
    * **Parâmetros:** `{"minutos_totais": 130}`
    * **Resposta:** `{"horas": 2, "minutos_restantes": 10}`

8. **Cálculo de Desconto:** Crie um endpoint `POST /api/loja/aplicar-desconto`.
    * **Parâmetros:** `{"valor_produto": 100, "porcentagem_desconto": 15}`
    * **Resposta:** `{"valor_com_desconto": 85.0}`

9. **Perímetro do Círculo:** Crie um endpoint `POST /api/geometria/circulo/perimetro`.
    * **Parâmetros:** `{"raio": 5}`
    * **Resposta:** `{"perimetro": 31.41}`

10. **Consumo de Veículo:** Crie um endpoint `POST /api/veiculos/consumo-medio`.
    * **Parâmetros:** `{"distancia_km": 500, "litros_combustivel": 50}`
    * **Resposta:** `{"km_por_litro": 10.0}`

11. **Conversor de Moeda:** Crie um endpoint `POST /api/financas/cambio`.
    * **Parâmetros:** `{"valor_real": 100, "cotacao_dolar": 5.20}`
    * **Resposta:** `{"valor_convertido_dolar": 19.23}`

12. **Idade em Dias:** Crie um endpoint `POST /api/pessoa/idade-dias`.
    * **Parâmetros:** `{"idade_anos": 20}`
    * **Resposta:** `{"dias_vividos_estimados": 7300}`

13. **Área do Triângulo:** Crie um endpoint `POST /api/geometria/triangulo/area`.
    * **Parâmetros:** `{"base": 10, "altura": 5}`
    * **Resposta:** `{"area": 25.0}`

14. **Calculadora de Gorjeta:** Crie um endpoint `POST /api/restaurante/conta`.
    * **Parâmetros:** `{"valor_consumo": 100.0, "porcentagem_gorjeta": 10}`
    * **Resposta:** `{"valor_gorjeta": 10.0, "total_a_pagar": 110.0}`

15. **Conversor de Segundos:** Crie um endpoint `POST /api/conversores/tempo-completo`.
...
16. **Soma de Array:** Crie um endpoint `POST /api/listas/somar`.
    * **Parâmetros:** `{"valores": [1, 2, 3, 4, 5]}`
    * **Resposta:** `{"soma_total": 15}`

17. **Média de Múltiplas Notas:** Crie um endpoint `POST /api/escola/media-turma`.
...
19. **Buscador do Maior Número:** Crie um endpoint `POST /api/listas/maior-valor`.
    * **Parâmetros:** `{"numeros": [45, 12, 99, 3, 88]}`
    * **Resposta:** `{"maior_numero_encontrado": 99}`

20. **Lista de Cumprimentos:** Crie um endpoint `POST /api/social/saudacoes`.
    * **Parâmetros:** `{"nomes": ["Ana", "Bruno", "Carlos"]}`
    * **Resposta:** `{"mensagens": ["Olá, Ana!", "Olá, Bruno!", "Olá, Carlos!"]}`

21. **Ordem Inversa da Lista:** Crie um endpoint `POST /api/listas/inverter`.
    * **Parâmetros:** `{"dados": [10, 20, 30, 40]}`
    * **Resposta:** `{"dados_invertidos": [40, 30, 20, 10]}`

22. **Combinação de Nomes:** Crie um endpoint `POST /api/pessoas/formatar-nomes`.
    * **Parâmetros:** `{"nomes": ["Ana", "Bruno"], "sobrenomes": ["Silva", "Gomes"]}`
    * **Resposta:** `{"nomes_completos": ["Ana Silva", "Bruno Gomes"]}`

23. **Inversão de String:** Crie um endpoint `POST /api/texto/inverter`.
    * **Parâmetros:** `{"palavra": "developer"}`
    * **Resposta:** `{"palavra_invertida": "repoleved"}`

24. **Cálculo de Matriz 3x3:** Crie um endpoint `POST /api/matematica/matriz/diagonal`.
    * **Parâmetros:** `{"matriz": [[1,2,3], [4,5,6], [7,8,9]]}`
    * **Resposta:** `{"soma_diagonal_principal": 15}`

25. **Contador de Palavras:** Crie um endpoint `POST /api/texto/frequencia`.
    * **Parâmetros:** `{"palavras": ["sol", "chuva", "sol", "nuvem"]}`
    * **Resposta:** `{"frequencia": {"sol": 2, "chuva": 1, "nuvem": 1}}`

26. **Contagem de Pares e Ímpares:** Crie um endpoint `POST /api/matematica/paridade`.
    * **Parâmetros:** `{"numeros": [1, 2, 3, 4, 5, 6]}`
    * **Resposta:** `{"total_pares": 3, "total_impares": 3}`

27. **Mecanismo de Busca Simples:** Crie um endpoint `POST /api/listas/buscar-item`.
